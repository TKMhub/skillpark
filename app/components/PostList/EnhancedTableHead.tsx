...
export default function EnhancedTable() {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = rows.map((n) => n.articleTitle);
  ...
  const handleClick = (
    ...
    const selectedIndex = selected.indexOf(articleTitle);
    let newSelected: readonly string[] = [];

    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selected, articleTitle);
          ...
          >
            ...
            <TableBody>
              {visibleRows.map((row, index) => {
                const isItemSelected = isSelected(row.articleTitle);
                const labelId = `enhanced-table-checkbox-${index}`;

                return (
                  <TableRow
                    hover
                    onClick={(event) => handleClick(event, row.articleTitle)}
                    role="checkbox"
